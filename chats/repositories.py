...
class ChatRepository:
    ...
    @staticmethod
    async def get_participant(session: AsyncSession, chat_id: uuid.UUID, user_id: uuid.UUID) -> ChatParticipant:
        result = await session.execute(
            select(ChatParticipant).where(ChatParticipant.user_id == user_id, ChatParticipant.chat_id == chat_id)
        )
        return result.scalars().first()

    @staticmethod
    async def get_participants(session: AsyncSession, chat_id: uuid.UUID) -> list[ChatParticipant]:
        result = await session.execute(
    ...
    @staticmethod
    async def remove_participant(session: AsyncSession, chat_id: uuid.UUID, user_id: uuid.UUID):
    ...
    @staticmethod
    async def change_role(session: AsyncSession, chat_id: uuid.UUID, user_id: uuid.UUID, new_role: ParticipantRole):
        participant = await ChatRepository.get_participant(session, chat_id, user_id)
        participant.role = new_role
        await session.flush()

    @staticmethod
    async def find_personal_chat(session: AsyncSession, user1: uuid.UUID, user2: uuid.UUID) -> Chat | None:
        result = await session.execute(
            select(Chat)
            .join(ChatParticipant)
            .where(Chat.type == ChatType.personal, ChatParticipant.user_id.in_([user1, user2]))
